...
figure("fig11_atrium.jpg", "피카소의 대형 발레 무대막(국내 첫 공개) ― 공간으로 만든 ‘첫인상’", width_cm=13)

# ══════════════════════════════════════════════════════
# 3. 고찰 (핵심)
# ══════════════════════════════════════════════════════
h1("3. 작품·작가 조사")
para("관람한 주요 작품을 양식의 흐름(탄생 → 분석 → 종합 → 오르피즘 → 확장)에 따라 조사·정리했다. "
     "전시 캡션과 미술사 자료를 종합한 것으로, 사진으로 작품명을 단정하기 어려운 경우는 '추정' 또는 "
     "'캡션 기준'으로 표기했다.")

h2("3.1 큐비즘의 탄생 ― 피카소의 초기 두상 (사진 2~4)")
para("파블로 피카소(1881~1973)는 조르주 브라크와 함께 1907~1908년 큐비즘을 창시했다. ‘큐비즘’이라는 "
     "이름 자체가 1908년 비평가 루이 보셀이 브라크의 풍경을 ‘작은 입방체(cube)’라 조롱한 데서 나왔다. "
     "그 출발점이 1907년 〈아비뇽의 처녀들〉인데, 사진 2의 1907년 두상은 바로 그 시기, 얼굴을 아프리카 "
     "가면처럼 몇 개의 평면으로 환원하던 실험의 산물이다. 명암으로 부피를 만들던 전통을 버리고 ‘면의 각도’로 "
     "입체를 표현하기 시작한 이 전환이 이후 모든 큐비즘의 씨앗이 된다. 사진 3·4의 두상·인물은 그 실험이 더 "
     "진전돼, 하나의 얼굴에 정면과 측면이 동시에 보이도록 재구성한 단계를 보여준다.")

h2("3.2 분석적 큐비즘 ― 형태의 해체 (사진 5~6)")
para("1909~1912년의 ‘분석적 큐비즘’ 시기에 피카소와 브라크는 대상을 잘게 쪼개 여러 시점을 한 화면에 겹쳐 "
     "놓았고, 색은 갈색·회색의 단색조로 절제했다. 색채라는 ‘방해물’을 걷어내고 오직 형태의 구조만 탐구하기 "
     "위해서였다. 사진 5·6의 회색 화면이 그 전형으로, 인물·악기·정물의 윤곽이 거의 식별 불가능한 추상의 "
     "문턱까지 분해돼 있다. 후대에 미친 영향이 가장 큰, 큐비즘의 가장 급진적인 국면이다.")

h2("3.3 종합적 큐비즘 ― 다시 ‘읽히는’ 화면 (사진 7)")
para("분석적 큐비즘이 추상에 지나치게 가까워지자, 1912년경 피카소와 브라크는 화면에 신문지·벽지·악보 같은 "
     "실제 종이를 붙이는 ‘파피에 콜레(papier collé)’와 콜라주를 도입했다. 이로써 색과 패턴, 알아볼 수 있는 "
     "기호가 화면으로 돌아왔다 ― 이것이 ‘종합적 큐비즘’이다. 해체에서 재구성으로 방향을 튼 것이다. 사진 7의 "
     "점묘·문양·패턴이 결합된 화면이 이 단계의 감각을 보여준다.")

h2("3.4 오르피즘 ― 소니아 들로네와 ‘색채의 큐비즘’ (사진 8~9)")
para("소니아 들로네(1885~1979)는 남편 로베르 들로네와 함께, 형태 해체에 집중한 본류 큐비즘에서 갈라져 나와 "
     ...
     "Cubism)’이라 불렀다. 동심원과 곡면이 부딪히며 만들어내는 색의 리듬이 특징이다. 퐁피두가 소장한 소니아의 "
     "대표작은 〈전기 프리즘(Prismes électriques)〉(1914)이며, 사진 8·9의 원형 색채 추상은 서명·연도로 보아 "
     "1930년대의 ‘리듬(Rythme)’ 계열로 추정된다. 회화에 머물지 않고 직물·패션·무대·디자인으로 색채 이론을 "
     "확장한 점에서, 순수미술과 응용미술의 경계를 허문 선구자로 평가된다 ― 마케팅·디자인 관점에서 특히 주목할 "
     "이력이다.")

h2("3.5 조르주 야쿨로프 〈Bull〉 (사진 10)")
para("조르주(게오르기) 야쿨로프(1884~1928)는 아르메니아계 러시아 화가·무대미술가로, 파리와 모스크바를 "
     "오가며 큐비즘·미래주의의 어휘를 러시아 아방가르드에 전한 인물이다. 1917년 모스크바 ‘카페 피토레스크’의 "
     "실내 장식으로 유명하다. 캡션상 〈Bull〉(패널에 유채, Asphodel Foundation 기증)로 표기된 이 작품은, "
     "질주하는 황소를 큐비즘·미래주의식 동세(動勢)로 분해해 속도와 힘을 표현한다. 본류 큐비즘이 정물·인물에 "
     "집중한 데 비해, 큐비즘 어휘가 러시아·동유럽으로 퍼지며 ‘운동’의 표현으로 변주된 사례다.")

h2("3.6 피카소의 발레 무대막 ― 전시의 하이라이트 (사진 11)")
para("아트리움 중앙에 걸린 대형 작품은 피카소가 발레 공연을 위해 그린 무대막으로, 이번 전시의 하이라이트이자 "
     "국내 첫 공개다. 피카소는 1917년 〈파라드(Parade)〉를 시작으로 디아길레프의 발레 뤼스(Ballets Russes) "
     "무대미술에 깊이 관여했고, 퐁피두가 소장한 대형 무대막(발레 〈메르퀴르(Mercure)〉, 1924 추정)이 그 "
     "대표적 산물이다. 회화가 미술관 액자를 벗어나 극장이라는 공공 공간으로 확장된 사례이자, 분석적 큐비즘의 "
     "해체를 지나 다시 인물의 윤곽이 또렷해진 피카소의 신고전주의적 변모도 함께 읽을 수 있다. 작품을 아트리움 "
     "중앙·고천장에 건 큐레이팅 자체가, 이 무대막의 ‘공간성’을 전시 동선의 첫 충격으로 활용한 영리한 선택이다.")

h2("3.7 코리아 포커스 ― 큐비즘과 한국 근대미술")
para("특별 섹션 ‘코리아 포커스’는 김환기·유영국·박래현·이수억·함대정 등 한국 근대 작가 11인의 21점을 통해, "
     "서구 큐비즘·아방가르드가 한국적 현실에서 어떻게 수용·변주됐는지 보여준다. 추상의 선구자 김환기와 "
     "유영국, 동양화의 재료로 큐비즘적 화면 분할을 시도한 박래현 등은, 외래 양식이 단순 모방이 아니라 ‘우리 "
     "조형 언어’로 번역된 과정을 증언한다.")
